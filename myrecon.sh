...
s3scan()
{
   ...
   python ./s3scanner.py --out-file "$domain"_s3scan.txt --dump "$domain"_unique.txt
   ...
   cp "$domain"_s3scan.txt /"$current"/"$domain"/"$subdirectory"/fuzzing/
...
dirbruteforce()
{
   cd "$current"
   echo "***************************************************************************************************"
   echo -e "\e[92m Dirsearch will start.."
   echo -e "\e[92m Wfuzz will be in action.."
   echo "***************************************************************************************************"
   ...
   do
	...
	echo "						"$domain"					    "
	echo "**********************************************************************************************"
	wfuzz -c -w /usr/share/wordlists/dirbuster/directory-list-lowercase-2.3-medium.txt -t 100 -u "$domain"/FUZZ --sc 200,204 --oF "$domain"_wfuzz.txt
   done
   cp "$domain"_wfuzz.txt ./"$domain"/"$subdirectory"/fuzzing/
   rm "$domain"_wfuzz.txt
   s3scan
}
...
vhostscan()
{
   ...
   for domain in `cat "$domain"_unique.txt`
   do
	ruby scan.rb --ip="$IP" --host="$domain" --ignore-http-codes=400,404,302,403 >> output.txt
	echo "*************************************************************************" >> output.txt
   done
   cp output.txt "$current"/"$domain"/"$subdirectory"/fuzzing/
   rm output.txt
   cd -
...
ipresolve()
{
   ...
   cp /root/scripts/bounty/Myrecon/"$domain"/"$subdirctory"/subdomains/knock_ip.txt /root/scripts/bounty/Myrecon
   cp /root/scripts/bounty/Myrecon/"$domain"/"$subdirctory"/subdomains/massdns_ip.txt /root/scripts/bounty/Myrecon
