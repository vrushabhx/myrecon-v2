...
#Hidden parameter discovery
  ...
  arjun -i arjun_input.txt -oT arjun_result.txt -t 50 --headers "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:72.0) Gecko/20100101 Firefox/72.0"
...
# Crawl all subdomains and filter the result
   cd "$current"/"$domain"/"$subdirectory"/subdomains/
   cat "$domain"_unique.txt | xargs -I@ -P10 sh -c 'gospider -s "@" -o ../URLs/crawl_data/ -a -r -c 10 -d 3'
   cd ../URLs/crawl_data/
   ...
   cat * | grep -v -E "(.jpg|.JPG|.png|.svg|.gif|.ttf|.css|.js|.pdf|.mp4|.mp3|.woff|.eot|.jpeg|.exe|.woff2)" | grep "=" | grep "other-sources" | cut -d " " -f 3 | qsreplace -a | tee -a ../spider_clean_2.txt
   cd ../
   if [ -z "$module" ]
   then
	vulnscan
